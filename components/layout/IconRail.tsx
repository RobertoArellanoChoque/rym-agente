...
import { ArrowLeftRight, ShoppingCart, Truck, BookOpen, LayoutDashboard, Bot, ChevronRight } from "lucide-react"
...
export function IconRail() {
  ...
  return (
    ...
    >
      {/* Brand stripe */}
      <div className="h-0.5 w-full shrink-0" style={{ background: "linear-gradient(90deg, #E52713 0%, rgba(229,39,19,0.3) 70%, transparent 100%)" }} />

      {/* Logo mark */}
      <div className="flex items-center h-12 shrink-0 px-3.5 gap-2.5 overflow-hidden">
        <svg viewBox="0 0 24 22" fill="none" className="w-5 h-5 shrink-0" aria-label="RyM">
        ...
        {modules.map(mod => {
          ...
          return (
            <Link
              ...
              style={{ background: isActive ? "#E52713" : "transparent" }}
              onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = "var(--sidebar-accent)" }}
              onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = "transparent" }}
            >
              <Icon className="h-4 w-4 shrink-0" style={{ color: isActive ? "#fff" : "oklch(0.60 0.01 264)" }} />
              {expanded && <span className="text-sm" style={{ color: isActive ? "#fff" : "oklch(0.60 0.01 264)" }}>{mod.label}</span>}
            </Link>
          )
        })}
      </nav>
      ...
      <div className="flex items-center pb-4 px-3.5 gap-3 shrink-0">
        <span className="relative flex h-2 w-2 shrink-0" title="Agente IA activo">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-60" />
          <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-400" />
        </span>
        {expanded && <span className="text-[11px] text-muted-foreground whitespace-nowrap">IA activa</span>}
        <button
