...
const STEP_LABELS = ["Leyendo PDF", "Procesando con OCR", "Analizando cargos", "Guardando resultado"]

interface TarjetaUploadProps {
  processing: boolean
  stepIndex: number
  error?: string | null
...
export function TarjetaUpload({ processing, stepIndex, error, onUpload }: TarjetaUploadProps) {
  ...
  return (
    <div className="space-y-3 max-w-md">
      <p className="text-sm text-muted-foreground">
        Subí el PDF del resumen de la tarjeta. El agente extrae los cargos automáticamente.
      </p>
      ...
      {processing ? (
        <div className="rounded-lg border bg-muted/20 px-4 py-4">
          ...
          <LoadingSteps steps={buildSteps(STEP_LABELS, stepIndex)} />
        </div>
      ) : (
