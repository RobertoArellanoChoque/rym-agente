...
export async function GET(req: NextRequest) {
  ...
  try {
    const rows = modulo
      ? await db.select().from(sesiones).where(eq(sesiones.modulo, modulo)).orderBy(desc(sesiones.updatedAt))
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.insert(sesiones).values({ id, modulo, label: defaultLabel, estado: "activo", datos: {}, createdAt: now, updatedAt: now })
